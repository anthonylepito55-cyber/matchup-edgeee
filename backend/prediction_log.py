...
def get_logged_fav_sub3(date: str, game_pk: int) -> dict | None:
    """Same contract as get_logged_model_e_bet, for the sub-bar favorite tracker."""
    log = _read_log()
    if log.empty or "fav_sub3_json" not in log.columns:
        return None
    row = log[(log["date"] == date) & (log["game_pk"] == game_pk)]
    if row.empty:
        return None
    val = row.iloc[0].get("fav_sub3_json")
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return None
    try:
        return json.loads(val)
...
def get_model_e_track_record() -> dict:
    ...
    favsub_rows = []
    if "fav_sub3_json" in log.columns:
        for _, r in log[(log["settled"] == True) & log["fav_sub3_json"].notna()].iterrows():  # noqa: E712
            try:
                fb = json.loads(r["fav_sub3_json"])
            except (TypeError, ValueError):
                continue
            if not fb or r["home_won"] is None or pd.isna(r["home_won"]):
                continue
            won = bool(r["home_won"]) == bool(fb.get("side_is_home"))
            dec = model_e.american_to_decimal(fb.get("best_price"))
            favsub_rows.append({"won": won, "flat": ((dec - 1.0) if won else -1.0) if dec is not None else None})
    fav_sub3 = None
    if favsub_rows:
