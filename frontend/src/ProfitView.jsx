          ...
          const rows = noBet.map(g => {
            ...
            if (p != null && mkt != null) {
              ...
              if (epf - mkf <= -0.03 && mkf >= 0.55) {
                heavyDislike = { fav: fh ? g.home_team_abbr : g.away_team_abbr,
                                 dog: fh ? g.away_team_abbr : g.home_team_abbr,
                                 gap: (mkf - epf) * 100, band: mkf >= 0.60 ? 'heavy' : 'mid' }
              }
          ...
          return (
            <div style={{ marginTop: 14 }}>
              ...
              {rows.map(({ g, reason, detail, heavyDislike, closeDog, bbDog, favEdge }) => {
                ...
                // COLOR RULE (2026-09-24): green = two-season backtest-validated (bettable);
                // grey = backtest-negative / unproven watch band. The MID-dislike->DOG cell is
                // the only validated one down here; everything else greys out.
                  ...
                  <div key={`nobet-${g.game_pk}`} className="mono" style={{ display: 'grid', gridTemplateColumns: '1fr 260px 110px', gap: 10, alignItems: 'center', fontSize: 11, padding: '6px 6px', borderBottom: '1px solid var(--line)', color: 'var(--text-tertiary)', background: rowGreen ? 'rgba(63,185,80,0.12)' : anyMarker ? 'rgba(139,148,158,0.10)' : 'transparent', borderLeft: `3px solid ${rowGreen ? '#3fb950' : anyMarker ? '#8b949e' : 'transparent'}` }}>
                    <span>
                      ...
                      {heavyDislike && heavyDislike.band === 'mid' ? <span style={{ color: '#3fb950', fontWeight: 700, marginLeft: 6 }}
                        title={`MID-DISLIKE → TAKE THE DOG (two-season backtest-VALIDATED): ${heavyDislike.fav} is a 55-60% favorite the model dislikes by ${heavyDislike.gap.toFixed(1)} pts — the model is right, take the DOG ${heavyDislike.dog}. Backtest 2026 +16.4% / 2025 +13.5% (486 games, both halves both seasons). Live so far: ${mdd ? `${mdd.live_roi_pct > 0 ? '+' : ''}${mdd.live_roi_pct}% (${mdd.live_n})${mdd.recent_roi_pct != null ? `, rec ${mdd.recent_roi_pct > 0 ? '+' : ''}${mdd.recent_roi_pct}%` : ''}` : 'accruing'} — currently in the September dog drawdown, but validated across two seasons. Flat 1u.`}>≡ TAKE DOG: {heavyDislike.dog} <span style={{ fontWeight: 400, opacity: 0.9 }}>· 2yr +15% · live {mdd ? `${mdd.live_roi_pct > 0 ? '+' : ''}${mdd.live_roi_pct}% (${mdd.live_n})` : '…'}</span></span>
                        : heavyDislike ? <span style={{ color: '#8b949e', fontWeight: 700, marginLeft: 6 }} title={`HD band (grey = failed): 60%+ fav the model dislikes. The 60%+ version failed the 2025 backtest (−4.0%) — unlike the MID 55-60% version, which validated. No bet.`}>◦ HD (failed 2yr): {heavyDislike.fav}</span> : null}
                    </span>
            ...
            <div style={{ marginTop: 14, padding: '14px 18px', borderRadius: 8, border: `1px solid ${blue}`, background: 'rgba(88,166,255,0.05)' }}>
              ...
              {rows.map(r => {
                ...
                const ECFG = {
                  ...
                  micro_split: { lbl: `⚔ even+split → fade tracker: ${fadeTeam || 'starter side'}`, c: '#a3e635', tip: `TRACKED FADE (pre-registered 2026-09-17): starters nearly even but the slight starter edge is on ${fadeTeam || 'the other team'}, not the pen team. The tracker follows the STARTER side (${fadeTeam || '?'}) — current record: ${ssfTxt || 'accruing'}. HONESTY: the cell that motivated this looked +7.1% on a stale data snapshot and reads NEGATIVE on the site's own current data (see the 'reg' number) — a live lesson in noise-grade cells, which is exactly why this is a TRACKER and not advice. Judged only on post-registration games, checkpoint 50. Not staked, not in the risk total; do not bet it while 'reg' is negative.` },
              ...
              <div className="mono" style={{ fontSize: 9, color: 'var(--text-tertiary)', marginTop: 4 }}>
                blue = the bucket's live record is positive so far · <span style={{ color: '#f85149' }}>red rows = negative CONFIGURATION</span> (big-gap split: both sides lose live) · <span style={{ color: '#a3e635' }}>lime rows = even+split FADE TRACKER</span> — follows the named starter-side team (pre-registered 9/17; the motivating +7.1% cell read NEGATIVE on current data the same day it was registered — hover the marker for the live record; checkpoint 50 post-registration; not a pick while its 'reg' number is negative) · <span style={{ color: '#e3b341' }}>≡ stacked</span> / <span style={{ color: '#2563eb' }}>≡ even+edges</span> = the aligned configs (63.3%/+6.6% and 58.7%/+12.4% live) · ⚠ opener = the "starter" won't pitch bulk, distrust the bucket label. All slices inside noise bands; the profitable version of the bullpen signal is still the model-confirmed one (PEN+WHIP ✓ on the slip).
              </div>
            </div>
          )
        })()}
